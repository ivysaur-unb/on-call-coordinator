                ...
                <div>
                    <Table size="small" sx={{ width: "100%" }}>
                        <TableHead sx={{ fontWeight: "bold" }}>
                            <TableRow>
                                <TableCell align={"center"} colSpan={1}>Teacher</TableCell>
                                <TableCell align={"center"} colSpan={1}>Edit</TableCell>
                            ...
                            <TableRow>
                                <TableCell></TableCell>
                                <TableCell></TableCell>
                                {[...Array(20)].map((e, i) => <TableCell key={i}>P<sub>{(i % 4) + 1}</sub></TableCell>)}
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {teachers ? teachers.map((absent, index) => {
                                ...
                                return (
                                    <TableRow key={absent.id}>
                                        ...
                                        <TableCell><label style={{color:"blue",fontWeight:"bold"}}>Edit<input style={{display:"none"}} type="button" value={absent.id} onClick={(e) => {
                                            handleChange(e);
                                            onClick(e);
                                        }} /></label></TableCell>
                                        {[...Array(20)].map((e, i) => {
                                            ...
                                            console.log({weekAbsences})
                                            return <TableCell key={i} style={{ backgroundColor: (weekAbsences[absent.id] && weekAbsences[absent.id].some(x => new Date(x.day).toDateString() === day.toDateString() && x.period === ((i % 4) + 1)) ? 'black' : 'white') }}></TableCell>
                                        })}
...
const saveAbsences = (absences, dateStart) => {
    ...
    for (let i = 0; i < absences.length; i++) {//each teacher in the array
        ...
        tempArr[absences[i].id] = item.filter(x => new Date(x.day) >= nextDay && new Date(x.day) <= fri)
